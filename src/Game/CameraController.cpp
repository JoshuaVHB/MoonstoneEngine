...
void CameraController::computeThirdPersonPosition(const Cloporte& player, Camera& cam)
{
	...
	const float computedHeight = std::clamp(computeCameraHeight(player, cam), -30.f, 30.f);
...
float CameraController::computeCameraHeight(const Cloporte& player, const Camera& cam)
{
	const auto forwardDir = player.getForward();
...
void CameraController::computeFirstPersonPosition(const Cloporte& player, Camera& cam)
{
	...
	// unused : static constexpr float MAX_SLERP = 10.f;
	const XMVECTOR lerpedForward = XMVectorLerp(
		cam.getForwardDir(),
		player.getForward(),
		smoothstep(0, 1, XMVectorGetX(XMVector3Length(player.getForward() - cam.getForwardDir()))
		));

	cam.lookAt(lerpedForward + player.getPosition() + XMVECTOR{0.F, 2.F, 0.F, 0.F});
	cam.updateCam();
}
